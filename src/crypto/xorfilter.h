...
// Seeded variant of xor8_populate — accepts an explicit initial seed
...
#endif
static inline bool xor8_populate_seeded(uint64_t *keys, uint32_t size,
                                        xor8_t *filter, uint64_t initial_seed) {
  if(size == 0) { return false; }
  uint64_t rng_counter = initial_seed;
  filter->seed = xor_rng_splitmix64(&rng_counter);
  size_t arrayLength = (size_t)(filter->blockLength) * 3;
  size_t blockLength = (size_t)(filter->blockLength);
  ...
  while (true) {
    ...
    if(iterations > XOR_SEEDED_MAX_ITERATIONS) {
      free(sets);
      free(Q);
      free(stack);
      return false;
    ...
    while (Q0size + Q1size + Q2size > 0) {
      while (Q0size > 0) {
        xor_keyindex_t keyindex = Q0[--Q0size];
        size_t index = keyindex.index;
        if (sets0[index].count == 0) continue;
        uint64_t hash = keyindex.hash;
        uint32_t h1 = xor8_get_h1(hash, filter);
        uint32_t h2 = xor8_get_h2(hash, filter);
        stack[stack_size] = keyindex;

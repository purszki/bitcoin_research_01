...
// Seeded variant of binary_fuse8_populate — accepts an explicit initial seed
// for deterministic construction. Uses higher iteration limit (1000).
#ifndef XOR_SEEDED_MAX_ITERATIONS
#define XOR_SEEDED_MAX_ITERATIONS 1000
#endif
static inline bool binary_fuse8_populate_seeded(uint64_t *keys, uint32_t size,
                           binary_fuse8_t *filter, uint64_t initial_seed) {
  if (size != filter->Size) {
    return false;
  }

  uint64_t rng_counter = initial_seed;
  filter->Seed = binary_fuse_rng_splitmix64(&rng_counter);
  uint64_t *reverseOrder = (uint64_t *)calloc((size + 1), sizeof(uint64_t));
  uint32_t capacity = filter->ArrayLength;
  uint32_t *alone = (uint32_t *)malloc(capacity * sizeof(uint32_t));
  ...
  reverseOrder[size] = 1;
  for (int loop = 0; true; ++loop) {
    if (loop + 1 > XOR_SEEDED_MAX_ITERATIONS) {
      free(alone);
      free(t2count);
      free(reverseH);
      free(t2hash);
...
// Variant of binary_fuse16_populate that accepts an external initial seed
// for the RNG, allowing deterministic-per-input construction.
// Uses a higher iteration limit (1000) to handle edge-case element counts
// where the default 100 iterations may be insufficient.
#ifndef XOR_SEEDED_MAX_ITERATIONS
#define XOR_SEEDED_MAX_ITERATIONS 1000
#endif
static inline bool binary_fuse16_populate_seeded(uint64_t *keys, uint32_t size,
                           binary_fuse16_t *filter, uint64_t initial_seed) {
  if (size != filter->Size) {
    return false;
  }
...
static inline size_t binary_fuse16_serialization_bytes(binary_fuse16_t *filter) {
  // Must match what binary_fuse16_serialize() actually writes.
  // SegmentLengthMask is derived (= SegmentLength - 1), not serialized.

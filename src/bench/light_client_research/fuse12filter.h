...
#ifndef BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE12FILTER_H
#define BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE12FILTER_H
...
public:
    Fuse12Filter();
    ~Fuse12Filter();
    Fuse12Filter(Fuse12Filter&&) noexcept;
    Fuse12Filter& operator=(Fuse12Filter&&) noexcept;

    /** Build a filter from elements, keyed by siphash parameters. */
    Fuse12Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements);

    /** Check if a single element may be in the set. FPR = 1/4096. */
    ...
    static Fuse12Filter Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                    const std::vector<unsigned char>& data);
};

#endif // BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE12FILTER_H

...
#ifndef BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE18FILTER_H
#define BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE18FILTER_H
...
public:
    Fuse18Filter();
    ~Fuse18Filter();
    Fuse18Filter(Fuse18Filter&&) noexcept;
    Fuse18Filter& operator=(Fuse18Filter&&) noexcept;

    /** Build a filter from elements, keyed by siphash parameters. */
    Fuse18Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements);

    /** Check if a single element may be in the set. FPR = 1/262144. */
    ...
    static Fuse18Filter Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                    const std::vector<unsigned char>& data);
};

#endif // BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE18FILTER_H

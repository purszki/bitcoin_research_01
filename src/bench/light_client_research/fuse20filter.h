...
#ifndef BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE20FILTER_H
#define BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE20FILTER_H
...
public:
    Fuse20Filter();
    ~Fuse20Filter();
    Fuse20Filter(Fuse20Filter&&) noexcept;
    Fuse20Filter& operator=(Fuse20Filter&&) noexcept;

    /** Build a filter from elements, keyed by siphash parameters. */
    Fuse20Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements);

    /** Check if a single element may be in the set. FPR = 1/1048576. */
    ...
    static Fuse20Filter Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                    const std::vector<unsigned char>& data);
};

#endif // BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE20FILTER_H

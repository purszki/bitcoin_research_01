...
#ifndef BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE10FILTER_H
#define BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE10FILTER_H
...
public:
    Fuse10Filter();
    ~Fuse10Filter();
    Fuse10Filter(Fuse10Filter&&) noexcept;
    Fuse10Filter& operator=(Fuse10Filter&&) noexcept;

    /** Build a filter from elements, keyed by siphash parameters. */
    Fuse10Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements);

    /** Check if a single element may be in the set. FPR = 1/1024. */
    ...
    static Fuse10Filter Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                    const std::vector<unsigned char>& data);
};

#endif // BITCOIN_BENCH_LIGHT_CLIENT_RESEARCH_FUSE10FILTER_H

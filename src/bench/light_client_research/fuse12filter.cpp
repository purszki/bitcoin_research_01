...
// C++ header — only included here, never in fuse12filter.h.
#include <bench/xor_singleheader/binaryfusefilter.hpp>

struct Fuse12Filter::Impl {
    BinaryFuseFilter12 filter{};
...
uint64_t Fuse12Filter::HashElement(const Element& element) const
{
...
Fuse12Filter::Fuse12Filter()
    : m_impl{std::make_unique<Impl>()} {}

Fuse12Filter::~Fuse12Filter() = default;
Fuse12Filter::Fuse12Filter(Fuse12Filter&&) noexcept = default;
Fuse12Filter& Fuse12Filter::operator=(Fuse12Filter&&) noexcept = default;

Fuse12Filter::Fuse12Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements)
    : m_impl{std::make_unique<Impl>()}, m_siphash_k0(siphash_k0), m_siphash_k1(siphash_k1)
{
    if (elements.size() < 2) {
        throw std::invalid_argument("Fuse12Filter requires at least 2 elements, got "
    ...
    if (!m_impl->filter.allocate(n)) {
        throw std::runtime_error("Fuse12Filter: allocation failed");
    }
    ...
    if (!m_impl->filter.populate_seeded(keys.data(), n, initial_seed)) {
        m_impl->filter.free();
        throw std::runtime_error("Fuse12Filter: construction failed after max iterations");
    }

    m_impl->populated = true;
}

bool Fuse12Filter::Match(const Element& element) const
{
...
bool Fuse12Filter::MatchAny(const ElementSet& elements) const
{
    if (!m_impl || !m_impl->populated) return false;
    for (const Element& elem : elements) {
...
size_t Fuse12Filter::SerializedSize() const
{
    if (!m_impl || !m_impl->populated) return 0;
    return m_impl->filter.serialization_bytes();
}

std::vector<unsigned char> Fuse12Filter::Serialize() const
{
...
Fuse12Filter Fuse12Filter::Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                       const std::vector<unsigned char>& data)
{
    static constexpr size_t HEADER_SIZE = 28;
    if (data.size() < HEADER_SIZE) {
        throw std::runtime_error("Fuse12Filter: buffer too small for header");
    }

    uint32_t array_length;
    std::memcpy(&array_length, data.data() + HEADER_SIZE - sizeof(uint32_t), sizeof(uint32_t));

    // 12-bit packed fingerprints: ceil(array_length * 12 / 8) bytes
    const size_t fp_bytes = (static_cast<uint64_t>(array_length) * 12 + 7) / 8;
    const size_t expected_size = HEADER_SIZE + fp_bytes;
    if (data.size() < expected_size) {
        throw std::runtime_error("Fuse12Filter: buffer too small for fingerprints ("
                                 + std::to_string(data.size()) + " < " + std::to_string(expected_size) + ")");
    }
    if (data.size() != expected_size) {
        throw std::runtime_error("Fuse12Filter: buffer size mismatch (trailing data), got "
    ...
    if (!result.m_impl->filter.deserialize(reinterpret_cast<const char*>(data.data()))) {
        throw std::runtime_error("Fuse12Filter: deserialization failed");
    }

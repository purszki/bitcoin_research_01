...
// C++ header — only included here, never in fuse20filter.h.
#include <bench/xor_singleheader/binaryfusefilter.hpp>

struct Fuse20Filter::Impl {
    BinaryFuseFilter20 filter{};
...
uint64_t Fuse20Filter::HashElement(const Element& element) const
{
...
Fuse20Filter::Fuse20Filter()
    : m_impl{std::make_unique<Impl>()} {}

Fuse20Filter::~Fuse20Filter() = default;
Fuse20Filter::Fuse20Filter(Fuse20Filter&&) noexcept = default;
Fuse20Filter& Fuse20Filter::operator=(Fuse20Filter&&) noexcept = default;

Fuse20Filter::Fuse20Filter(uint64_t siphash_k0, uint64_t siphash_k1, const ElementSet& elements)
    : m_impl{std::make_unique<Impl>()}, m_siphash_k0(siphash_k0), m_siphash_k1(siphash_k1)
{
    if (elements.size() < 2) {
        throw std::invalid_argument("Fuse20Filter requires at least 2 elements, got "
    ...
    if (!m_impl->filter.allocate(n)) {
        throw std::runtime_error("Fuse20Filter: allocation failed");
    }
    ...
    if (!m_impl->filter.populate_seeded(keys.data(), n, initial_seed)) {
        m_impl->filter.free();
        throw std::runtime_error("Fuse20Filter: construction failed after max iterations");
    }

    m_impl->populated = true;
}

bool Fuse20Filter::Match(const Element& element) const
{
...
bool Fuse20Filter::MatchAny(const ElementSet& elements) const
{
    if (!m_impl || !m_impl->populated) return false;
    for (const Element& elem : elements) {
...
size_t Fuse20Filter::SerializedSize() const
{
    if (!m_impl || !m_impl->populated) return 0;
    return m_impl->filter.serialization_bytes();
}

std::vector<unsigned char> Fuse20Filter::Serialize() const
{
...
Fuse20Filter Fuse20Filter::Deserialize(uint64_t siphash_k0, uint64_t siphash_k1,
                                       const std::vector<unsigned char>& data)
{
    ...
    if (data.size() < HEADER_SIZE) {
        throw std::runtime_error("Fuse20Filter: buffer too small for header");
    }
    ...
    // 20-bit packed fingerprints: ceil(array_length * 20 / 8) bytes
    const size_t fp_bytes = (static_cast<uint64_t>(array_length) * 20 + 7) / 8;
    const size_t expected_size = HEADER_SIZE + fp_bytes;
    if (data.size() < expected_size) {
        throw std::runtime_error("Fuse20Filter: buffer too small for fingerprints ("
                                 + std::to_string(data.size()) + " < " + std::to_string(expected_size) + ")");
    }
    if (data.size() != expected_size) {
        throw std::runtime_error("Fuse20Filter: buffer size mismatch (trailing data), got "
    ...
    if (!result.m_impl->filter.deserialize(reinterpret_cast<const char*>(data.data()))) {
        throw std::runtime_error("Fuse20Filter: deserialization failed");
    }

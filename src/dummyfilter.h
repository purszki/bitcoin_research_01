...
#ifndef BITCOIN_DUMMYFILTER_H
#define BITCOIN_DUMMYFILTER_H
 ...
 * DUMMY version for research.
 */
class GCSFilterDummy
{
public:
...
public:
    ...
    explicit GCSFilterDummy(const Params& params = Params());

    /** Reconstructs an already-created filter from an encoding. */
    GCSFilterDummy(const Params& params, std::vector<unsigned char> encoded_filter, bool skip_decode_check);

    /** Builds a new filter from the params and set of elements. */
    GCSFilterDummy(const Params& params, const ElementSet& elements);
 ...
 * DUMMY version for research.
 */
class BlockFilterDummy
{
private:
    BlockFilterType m_filter_type = BlockFilterType::INVALID;
    uint256 m_block_hash;
    GCSFilterDummy m_filter;

    bool BuildParams(GCSFilterDummy::Params& params) const;

public:

    BlockFilterDummy() = default;

    //! Reconstruct a BlockFilterDummy from parts.
    BlockFilterDummy(BlockFilterType filter_type, const uint256& block_hash,
                std::vector<unsigned char> filter, bool skip_decode_check);

    //! Construct a new BlockFilterDummy of the specified type from a block.
    BlockFilterDummy(BlockFilterType filter_type, const CBlock& block, const CBlockUndo& block_undo);

    BlockFilterType GetFilterType() const { return m_filter_type; }
    const uint256& GetBlockHash() const LIFETIMEBOUND { return m_block_hash; }
    const GCSFilterDummy& GetFilter() const LIFETIMEBOUND { return m_filter; }
    ...
    void Unserialize(Stream& s) {
        ...
        m_filter = GCSFilterDummy(params, std::move(encoded_filter), /*skip_decode_check=*/false);
    }
};

#endif // BITCOIN_DUMMYFILTER_H
